...
class Geocoder:
    ...
    def geocode(self, address: str) -> Optional[Tuple[float, float]]:
        ...
        with self.repo.Session() as session:
            # check in clean_listings for existing geocoded address
            # (In a real app, you might have a dedicated geocoding table)
            pass

        # Fast mode check - if we're in a hurry and it's not cached, skip external APIs
        if os.environ.get("GEOCAPE_FORCE_FAST") == "1":
            return None

        # L3: Providers (External APIs - slow path)
