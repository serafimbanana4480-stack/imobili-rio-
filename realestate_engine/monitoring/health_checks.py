    ...
    def check_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.db_repo.Session() as session:
                ...
                session.execute(text("SELECT COUNT(*) FROM raw_listings"))
    ...
    def check_external_apis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with httpx.Client(timeout=5.0) as client:
                response = client.get("https://api.ine.pt/", follow_redirects=True)
                latency_ms = (time.time() - start) * 1000
        ...
        try:
            ...
            start = time.time()
            with httpx.Client(timeout=5.0) as client:
                response = client.get("https://nominatim.openstreetmap.org/search?q=Porto&format=json", timeout=5.0)
                latency_ms = (time.time() - start) * 1000
