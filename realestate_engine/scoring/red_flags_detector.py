...
class RedFlagsDetector:
    ...
    def detect_detailed(cls, listing: Dict, valuation: Optional[Dict] = None) -> List[RedFlag]:
        ...
        if area and area > 0:
            p_m2 = preco / area
            if p_m2 < config.red_flag_price_m2_critical:
                flags.append(RedFlag(
                    "PRICE_TOO_LOW",
                    f"Preço/m² de {p_m2:.0f}€ suspeitamente baixo (possível fraude ou erro)",
                    "critical", 5.0
                ))
            elif p_m2 < config.red_flag_price_m2_warning:
                flags.append(RedFlag(
                    "PRICE_VERY_LOW",
        ...
        if valuation and valuation.get("discount") is not None:
            ...
            if discount < config.red_flag_overpriced_severe:
                flags.append(RedFlag(
                    "SEVERELY_OVERPRICED",
                    f"Sobrevalorizado em {abs(discount)*100:.0f}% acima do valor de mercado",
                    "warning", 2.0
                ))
            elif discount < config.red_flag_overpriced_moderate:
                flags.append(RedFlag(
                    "OVERPRICED",
        ...
        for kw in CRITICAL_KEYWORDS:
            if kw in full_text:
                flags.append(RedFlag(
                    ...
                    "critical", config.red_flag_price_m2_critical / 80  # penalty proportional to threshold
                ))
