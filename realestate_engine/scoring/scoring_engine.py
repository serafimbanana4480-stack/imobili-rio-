...
class ScoringEngine:
    ...
    def __init__(self, repo: Optional[DatabaseRepository] = None):
        ...
        # Use default weights if config not available
        try:
            weights = self.repo.get_config("scoring_weights")
            self.weights = weights if weights else WeightedScoreCalculator.DEFAULT_WEIGHTS.copy()
        except Exception:
            logger.warning("Could not load scoring_weights from config, using defaults")
            self.weights = WeightedScoreCalculator.DEFAULT_WEIGHTS.copy()
        ...
        self.weighted_calc = WeightedScoreCalculator(weights=self.weights)
        ...
        try:
            weights = self.repo.get_config("scoring_weights")
            if weights:
                self.weighted_calc = WeightedScoreCalculator(weights=weights)
        except Exception:
            pass
    ...
    def score(self, listing: CleanListing, valuation: Optional[Valuation] = None) -> Dict:
        ...
        if listing.num_fotos == 0:
            final_score = min(final_score, config.score_cap_no_photos)
        if listing.lat is None or listing.lon is None:
            final_score = min(final_score, config.score_cap_no_coords)
        if len(red_flag_strings) > 0:
            final_score = min(final_score, config.score_cap_with_flags)

        # Guard: If score is >= 9.0 but it doesn't meet the strict criteria, cap the score visually
        # This cap is applied BEFORE classification so that classificacao is consistent with the stored score.
        ...
        rationale = self.rationale_gen.generate(
            scores, red_flag_strings, listing_dict, valuation_dict, base_score=final_score
        )
    ...
    async def score_batch(self, batch_size: int = 500) -> int:
        ...
        # Purity guard — never score synthetic/sample data
        ...
        for listing in listings:
            ...
            existing = self.repo.get_score_by_listing(listing.id)
            if existing:
                continue

            # Get valuation (required for discount score)
            valuation = self.repo.get_valuation_by_listing(listing.id)
            if not valuation:
                continue
