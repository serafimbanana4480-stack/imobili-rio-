...
CLASSIFICATION_DISPLAY = {
    ...
    "Abaixo da média": {"emoji": "⬇️", "cor": "#999999", "headline": "OFERTA ABAIXO DA MÉDIA"},
...
class RationaleGenerator:
    ...
    def generate(
        ...
        If ``base_score`` is provided it is used directly (preferred).
        Otherwise the score is recalculated using the canonical
        ``WeightedScoreCalculator.DEFAULT_WEIGHTS``.
        """
        listing = listing or {}
        ...
        if base_score is not None:
            ...
            # Recalculate with canonical weights to avoid drift
            calc = WeightedScoreCalculator()
            total_score = calc.calculate(scores)

        parts = []

        # ── 1. Headline ────────────────────────────────────────────────
        classificacao = WeightedScoreCalculator.classify(total_score)
    ...
    def generate_telegram_summary(
        ...
        # Classification line
        if base_score is not None:
            classificacao = WeightedScoreCalculator.classify(base_score)
            lines.append(f"📊 {classificacao} ({base_score:.1f}/10)")
...
def _classify(score: float) -> str:
    """Legacy wrapper — delegates to canonical WeightedScoreCalculator."""
    return WeightedScoreCalculator.classify(score)
