...
- Neural Network model with PyTorch backend
- CatBoost model for categorical features
- Model caching for improved throughput
"""
import asyncio
import statistics
...
from realestate_engine.valuation.lightgbm_model import LightGBMModel
from realestate_engine.valuation.stacking_ensemble import StackingEnsemble
from realestate_engine.valuation.neural_network_model import NeuralNetworkModel
from realestate_engine.cache.model_cache import get_model_cache
...
class ValuationEngine:
    ...
    def __init__(self, repo: Optional[DatabaseRepository] = None):
        ...
        # Add professional trainer with proper validation
    ...
    def _train_models(self, listings: List[Dict]):
        """Train models on existing listings with proper train/test split."""
        if self._models_trained:
            return

        # Check minimum data requirement
        if len(listings) < self.trainer.MIN_LISTINGS:
            logger.warning(
                f"Insufficient data for training: {len(listings)} < {self.trainer.MIN_LISTINGS}. "
        ...
        try:
            ...
            # Cross-validation for best model (XGBoost + LightGBM)
            for cv_model, cv_name in [(self.xgboost, "xgboost"), (self.lightgbm, "lightgbm")]:
                try:
                    cv_results = self.trainer.cross_validate(cv_model, listings, n_splits=5)
                    logger.info(f"{cv_name} cross-validation: {cv_results}")
                except Exception as e:
                    logger.warning(f"{cv_name} cross-validation failed: {e}")

            # Log evaluation summary
            summary = self.trainer.get_evaluation_summary()
            logger.info(f"Model training complete. Evaluation summary: {summary}")
            ...
            if self.lightgbm.is_trained:
                logger.info(f"LightGBM R²={self.lightgbm.r_squared:.3f}" if self.lightgbm.r_squared else "LightGBM trained")
            if self.stacking.is_trained:
                logger.info(f"Stacking R²={self.stacking.r_squared:.3f}" if self.stacking.r_squared else "Stacking trained")
    ...
    def retrain(self):
        ...
        # Reset trainer results
        self.trainer.clear_results()
        ...
        if self.xgboost.model:
            with open(os.path.join(model_dir, "xgboost_latest.pkl"), "wb") as f:
                pickle.dump(self.xgboost.model, f)

        if self.hedonic.huber_model:
            with open(os.path.join(model_dir, "hedonic_latest.pkl"), "wb") as f:
                pickle.dump({
    ...
    def valuate_advanced(self, listing: Dict, pool: Optional[List[Dict]] = None) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                ...
                "hedonic_value": round(ensemble_result.individual_predictions.get("hedonic", ModelPrediction("", 0, 0)).prediction, 2),
                "comps_value": round(ensemble_result.individual_predictions.get("comps", ModelPrediction("", 0, 0)).prediction, 2),
                "ine_value": round(ensemble_result.individual_predictions.get("ine", ModelPrediction("", 0, 0)).prediction, 2),
                "xgboost_value": round(ensemble_result.individual_predictions.get("xgboost", ModelPrediction("", 0, 0)).prediction, 2),
                
                # Additional advanced models
                "neural_network_value": round(ensemble_result.individual_predictions.get("neural_network", ModelPrediction("", 0, 0)).prediction, 2),
                "catboost_value": round(ensemble_result.individual_predictions.get("catboost", ModelPrediction("", 0, 0)).prediction, 2),
                "random_forest_value": round(ensemble_result.individual_predictions.get("random_forest", ModelPrediction("", 0, 0)).prediction, 2),
                "linear_model_value": round(ensemble_result.individual_predictions.get("linear", ModelPrediction("", 0, 0)).prediction, 2),
            ...
            logger.warning(f"Advanced valuation failed for {listing.get('source_id', 'unknown')}: {e}. Falling back to standard 4-layer valuation.")
            # Fallback to standard valuation
            fallback_result = self.valuate(listing, pool)
            if fallback_result:
                fallback_result["valuation_source"] = "fallback_standard"
    ...
    def valuate(self, listing: Dict, pool: Optional[List[Dict]] = None) -> Optional[Dict]:
        ...
        lightgbm_val, lightgbm_explanation = self.lightgbm.predict_with_explanation(listing)
        stacking_val = self.stacking.predict(listing)
        comps_val, comps_conf = self.comps.predict(listing, pool or [])
        ine_val = self.ine.estimate_value(listing)
        ...
        # 1. Outlier Guard: If price/m2 is 3x higher than INE or average, it's risky
        price_m2 = listing.get("preco_por_m2") or (listing["preco_pedido"] / listing["area_util_m2"] if listing.get("area_util_m2") else 0)
        if ine_val and listing.get("area_util_m2"):
            ine_m2 = ine_val / listing["area_util_m2"]
            if price_m2 > ine_m2 * 2.5:
                logger.warning(f"Extreme price/m2 detected: {price_m2:.0f} vs INE {ine_m2:.0f}. Scaling down fair value.")
                fair_value = min(fair_value, ine_val * 1.5)

        # 2. Conservative Blend: Blend with INE based on confidence
        ensemble_conf = self.ensemble.calculate_confidence(valuations)
        if ine_val and ensemble_conf < 0.6:
            # Low confidence in ML models? Lean more towards INE (official data)
            weight_ine = 1.0 - ensemble_conf
            fair_value = (fair_value * ensemble_conf) + (ine_val * weight_ine)

        # 3. Low Confidence Margin
        if ensemble_conf < 0.4:
            # Apply a 10-20% safety margin depending on how low the confidence is
            margin = 0.80 + (ensemble_conf * 0.25) # 0.80 to 0.90
            fair_value = fair_value * margin
        ...
        return {
            ...
            "lightgbm_value": round(valuations["lightgbm"], 2) if valuations["lightgbm"] else None,
            "stacking_value": round(valuations["stacking"], 2) if valuations["stacking"] else None,
            "xgboost_explanation": xgboost_explanation,
            "lightgbm_explanation": lightgbm_explanation,
    ...
    def _build_valuation_quality(
        ...
        valid_models = [v for v in model_values.values() if v is not None and v > 0]
