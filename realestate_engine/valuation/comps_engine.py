...
# ── Adjustment factors for Porto market (2025-2026) ─────────────────────
...
ADJUSTMENT_FACTORS = {
    "garagem_add": 15_000,          # Having a garage adds ~15k
    "garagem_box_add": 20_000,      # Box garage adds ~20k
    "piscina_add": 18_000,          # Pool adds ~18k
    "terraço_add_per_m2": 800,      # Terrace value per m²
    "elevador_add": 8_000,          # Elevator adds ~8k
    "andar_add_per_floor": 2_500,   # Each extra floor adds ~2.5k
    "suite_add": 6_000,             # En-suite adds ~6k
    ...
    "quarto_extra_value": 18_000,   # Each extra bedroom: ~18k
    "wc_extra_value": 6_000,        # Each extra bathroom: ~6k
}
...
class CompsEngine:
    ...
    @staticmethod
    def _similarity_score(target: Dict, comp: Dict) -> float:
        ...
        if t_area > 0 and c_area > 0:
            ...
            score += (1.0 - min(area_diff, 1.0)) * 0.35
            total_weight += 0.35
        ...
        if t_freg and c_freg and t_freg == c_freg:
            score += 0.30  # Same parish = perfect
        elif t_conc and c_conc and t_conc == c_conc:
            score += 0.18  # Same municipality = good
        else:
            score += 0.0
        total_weight += 0.30
    ...
    def _adjusted_price(target: Dict, comp: Dict) -> float:
        ...
        Example: If the comp has a garage (+15k) but the target doesn't,
        we subtract 15k from the comp's price to estimate the target's value.
        """
        ...
        if comp_garage and not target_garage:
            adjustment -= ADJUSTMENT_FACTORS["garagem_add"]
        ...
        adjustment += room_diff * ADJUSTMENT_FACTORS["quarto_extra_value"]

        # Bathroom adjustment
        t_wc = target.get("casas_banho") or 1
        c_wc = comp.get("casas_banho") or 1
        wc_diff = t_wc - c_wc
        adjustment += wc_diff * ADJUSTMENT_FACTORS["wc_extra_value"]

        # Floor adjustment
        t_floor = target.get("andar") or 0
        c_floor = comp.get("andar") or 0
        floor_diff = t_floor - c_floor
        adjustment += floor_diff * ADJUSTMENT_FACTORS["andar_add_per_floor"]
        ...
        if t_estado in new_states and c_estado not in new_states:
            adjustment += comp_price * ADJUSTMENT_FACTORS["novo_premium_pct"]
        elif t_estado in reno_states and c_estado not in (new_states | reno_states):
            adjustment += comp_price * ADJUSTMENT_FACTORS["renovacao_premium_pct"]
        ...
        if t_cert in ("A+", "A") and c_cert not in ("A+", "A"):
            adjustment += comp_price * ADJUSTMENT_FACTORS["cert_a_premium_pct"]
        elif t_cert in ("F", "G") and c_cert not in ("F", "G"):
            adjustment += comp_price * ADJUSTMENT_FACTORS["cert_fg_discount_pct"]
    ...
    def find_comps_with_tiers(
        ...
        for other in pool:
            ...
            sim = self._similarity_score(listing, other)
            dist = float('inf')
