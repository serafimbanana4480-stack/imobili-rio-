...
class Orchestrator:
    ...
    def __init__(self):
        ...
        # Configuration
        self.silence_start_hour = 23  # 11 PM
        self.silence_end_hour = 8     # 8 AM
        
    def _is_silence_period(self) -> bool:
        """Check if current time is within the night silence window."""
        current_hour = datetime.now().hour
        if self.silence_start_hour <= current_hour or current_hour < self.silence_end_hour:
            return True
        return False
        ...
        try:
            ...
            try:
                async with JobLogger("etl.pipeline") as jl:
                    ...
                    jl.records_processed = await self.etl_pipeline.run(batch_size=10000)
            except Exception as e:
                ...
                if self._is_silence_period():
                    ...
                    async with JobLogger("notification.top_opps") as jl:
                        ...
                        sent = await self.notifier.notify_top_opportunities(
                            min_score=9.0, max_notifications=3,
                        )
