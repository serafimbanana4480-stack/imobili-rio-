...
class ValuationRequest(BaseModel):
    ...
    use_advanced: bool = Field(False, description="Use 8-model ensemble (slower but more accurate)")


class ValuationCreate(BaseModel):
    """Schema for creating a valuation record."""
    listing_id: str
    valor_justo: float
    ci_lower: float
    ci_upper: float
    ...
    hedonic_value: Optional[float] = None
    comps_value: Optional[float] = None
    ine_value: Optional[float] = None
    xgboost_value: Optional[float] = None
