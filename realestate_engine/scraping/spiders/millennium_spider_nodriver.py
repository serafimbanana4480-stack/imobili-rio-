"""Millennium BCP Spider - Bank-owned properties (desinvestimento bancário).

Millennium BCP lists repossessed properties through its
real estate division, often at competitive prices.
"""
import asyncio
import re
...
class MillenniumSpider(BaseSpiderNodriver):
    """Spider for Millennium BCP bank-owned properties."""

    name = "millennium"
    base_url = "https://www.millenniumbcp.pt/pt/Particulares/Creditos/Credito-Habitacao/imoveis-venda.html"
    sleep_range = (5, 12)
    max_pages = 5
    ...
    async def parse_page(self, page_num: int) -> List[Dict]:
        ...
        try:
            ...
            count = await self.wait_for_selector("div.property, div.imovel, article.property-card, div.listing", timeout=15.0)
            if count == 0:
                logger.warning(f"[{self.name}] No property items found on page {page_num}")
                return listings

            cards = await self.tab.query_selector_all("div.property, div.imovel, article.property-card, div.listing")
    ...
    async def _extract_card(self, card) -> Optional[Dict]:
        try:
            ...
            if url and not url.startswith("http"):
                url = f"https://www.millenniumbcp.pt{url}" if url.startswith("/") else f"https://www.millenniumbcp.pt/{url}"
