...
class REMAXDirectSpider:
    ...
    async def _fetch_detail(self, client: httpx.AsyncClient, url: str) -> Optional[Dict[str, Any]]:
        try:
            ...
            self.stats["errors"] += 1
            logger.debug(f"[{self.name}] detail {url}: {e.__class__.__name__}")
            return None

        if resp.status_code != 200:
            self.stats["errors"] += 1
            logger.debug(f"[{self.name}] detail {url}: HTTP {resp.status_code}")
            return None
    ...
    def _project(self, product: Dict[str, Any], url: str) -> Optional[Dict[str, Any]]:
        ...
        if price_value is None or price_value <= 0:
            print(f"[{self.name}] Skipped {url}: Invalid price {price}")
            self.stats["skipped"] += 1
            return None
        ...
        if area is None or rooms is None or area <= 0 or rooms < 0:
            print(f"[{self.name}] Skipped {url}: Area={area}, Rooms={rooms}")
            self.stats["skipped"] += 1
            return None
