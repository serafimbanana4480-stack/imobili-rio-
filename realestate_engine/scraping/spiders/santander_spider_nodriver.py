"""Santander Spider - Bank-owned properties (desinvestimento bancário).

Santander Totta lists repossessed properties through its
real estate division, often at below-market prices.
"""
import asyncio
import re
...
class SantanderSpider(BaseSpiderNodriver):
    """Spider for Santander bank-owned properties."""

    name = "santander"
    base_url = "https://www.santander.pt/pt_PT/Particulares/Creditos/Credito-Habitacao/imoveis-venda.html"
    sleep_range = (5, 12)
    max_pages = 5
    ...
    async def _extract_card(self, card) -> Optional[Dict]:
        try:
            ...
            if url and not url.startswith("http"):
                url = f"https://www.santander.pt{url}" if url.startswith("/") else f"https://www.santander.pt/{url}"
