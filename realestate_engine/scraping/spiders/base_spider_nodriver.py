...
try:
    ...
    NODRIVER_IMPORT_ERROR = None
except Exception as e:
    uc = None  # type: ignore[assignment]
    NODRIVER_AVAILABLE = False
    NODRIVER_IMPORT_ERROR = e
...
class BaseSpiderNodriver(ABC):
    ...
    def __init__(self, proxy_manager: Optional[ProxyManager] = None, health_monitor: Optional[ScrapingHealthMonitor] = None):
    ...
    async def start_browser(self, headless: bool = True):
        ...
        if not NODRIVER_AVAILABLE:
            raise RuntimeError(
                "nodriver is unavailable in this environment: "
                f"{NODRIVER_IMPORT_ERROR}"
            )
        ...
        logger.info(f"[{self.name}] Professional browser started (headless={headless}, proxy={proxy[:50] if proxy else 'none'})")

        # Record proxy usage in health monitor
        if self.health_monitor and proxy:
            self.health_monitor.record_proxy_use(proxy, success=True)
        ...
        try:
            ...
            for marker in markers:
                if marker.lower() in content.lower():
                    ...
                    # Record blocking detection in health monitor
                    if self.health_monitor:
                        self.health_monitor.record_proxy_use(
                            self.current_proxy or "none",
                            success=False
                        )
    ...
    async def handle_captcha(self) -> bool:
        """Try to handle captcha by waiting and retrying with improved strategy."""
        logger.warning(f"[{self.name}] Captcha detected, applying improved handling...")

        # Strategy 1: Wait and retry (existing)
        ...
        try:
            ...
            logger.warning(f"[{self.name}] Page reload failed: {e}")

        # Check if captcha is still there
        if await self.detect_blocking():
            logger.error(f"[{self.name}] Captcha still present after wait and reload")
            return False
        ...
        for attempt in range(self.max_retries):
            try:
                if self.is_blocked:
                    ...
                    if self.health_monitor and self.current_proxy:
                        self.health_monitor.record_proxy_use(self.current_proxy, success=False)
