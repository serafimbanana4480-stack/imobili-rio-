...
def _render_view(module_path: str, func_name: str) -> None:
    ...
    try:
        ...
        st.error(f"Erro em `{module_path}.{func_name}`: {type(exc).__name__}: {exc}")
        with st.expander("🔍 Traceback completo (debug)", expanded=False):
...
if theme == '☀️ Claro':
    ...
    <style>
        ...
        .re-card { background: white !important; }
    </style>
    """, unsafe_allow_html=True)
else:
    ...
    <style>
        ...
        /* Status badge */
        .re-badge-success { background: #064E3B !important; color: #ECFDF5 !important; }
        .re-badge-warning { background: #78350F !important; color: #FFFBEB !important; }
        .re-badge-error   { background: #7F1D1D !important; color: #FEF2F2 !important; }
        .re-badge-info    { background: #1E3ADA !important; color: #EFFFFF !important; }
...
# Check session_state first for programmatic navigation
...
# Quick actions
if st.sidebar.button("🔍 Pesquisar Tudo", use_container_width=True):
    st.session_state.page = "🔍 Pesquisar"
    st.rerun()

if st.sidebar.button("⭐ Ver Watchlist", use_container_width=True):
    st.session_state.page = "⭐ Watchlist"
    st.rerun()

if st.sidebar.button("🔥 Top Oportunidades", use_container_width=True):
    st.session_state.page = "🏠 Início"
    st.rerun()

st.sidebar.markdown("---")
st.sidebar.caption(f"📁 `{config.data_dir}`")
...
try:
    ...
    st.sidebar.markdown(
        ...
        f"<b>🏠 Imóveis reais</b>: {_total_real:,}"
        f"</div>".replace(",", "."),
        unsafe_allow_html=True,
    )
    if _total_real == 0:
...
if not st.session_state.onboarding_completed:
    with st.expander("👋 Bem-vindo ao Porto RE Engine!", expanded=True):
        ...
        **🏠 Início** - Visão geral do mercado com KPIs e melhores oportunidades
        
        **🔍 Pesquisar** - Encontre propriedades com filtros avançados
        
        **⭐ Watchlist** - Guarde propriedades para análise futura
        
        **📊 Análise** - Mapa, análise de mercado e ferramentas de investimento
        
        **⚙️ Sistema** - Estado do sistema, scraping e configurações
        
        ---
        
        💡 **Dica:** Use o toggle de tema ☀️/🌙 no topo da sidebar para mudar entre modo claro e escuro.
        """)
        
        if st.button("✓ Entendi, começar!", use_container_width=True):
...
if page == "🏠 Início":
    ...
    analysis_pages = ["📈 Mercado", "🗺️ Mapa", "💼 Investidor", "🤖 Melhores Deals (IA)"]
    ...
    system_pages = ["🖥️ Estado", "🔄 Scraping", "🏆 Score Audit", "📱 Telegram", "⚙️ Config", "🔄 Pipeline", "📊 Qualidade", "🐛 Debug"]
