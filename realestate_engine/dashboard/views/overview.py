...
def _muted_color() -> str:
    return TEXT_COLOR_MUTED_DARK if is_dark_mode() else TEXT_COLOR_MUTED_LIGHT


def render_overview():
    """Render investment-focused overview dashboard."""
    st.title("🏠 Porto Real Estate Intelligence")
    st.markdown(f"<p style='color:{_muted_color()};font-size:1.1em;margin-top:-10px;'>Painel de decisão para investimento imobiliário — oportunidades em tempo real</p>", unsafe_allow_html=True)
    ...
    with qcol1:
        if st.button("💰 Melhores Oportunidades", use_container_width=True, type="primary"):
            st.session_state['min_score'] = 7.5
            st.session_state['max_price'] = 500000
            st.session_state['page'] = "🔍 Pesquisar"
            st.rerun()
    with qcol2:
        if st.button("🏠 T2 Porto Centro", use_container_width=True):
            st.session_state['tipologia'] = "T2"
            st.session_state['freguesia'] = "Cedofeita"
            st.session_state['page'] = "🔍 Pesquisar"
            st.rerun()
    with qcol3:
        if st.button("📉 Grande Desconto", use_container_width=True):
    ...
    with qcol4:
        if st.button("🗺️ Ver no Mapa", use_container_width=True):
            ...
            st.session_state['analysis_page'] = "🗺️ Mapa"
            st.rerun()
    with qcol5:
    ...
    st.markdown(f"<p style='color:{_muted_color()};margin-top:-10px;'>Imóveis com maior lucro potencial estimado — clique para detalhes completos</p>", unsafe_allow_html=True)
                ...
                with col:
                    ...
                    <div class="re-card">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                            ...
                            <div style="font-size:0.8em;color:{_muted_color()};">{score.classificacao or ""}</div>
                        </div>
                        ...
                        <div style="font-size:0.85em;color:{_muted_color()};margin-bottom:8px;">
    ...
    with st.expander("📋 Lista Detalhada de Oportunidades", expanded=True):
        st.markdown(f"<p style='color:{_muted_color()};margin-top:-10px;'>Todas as propriedades com score e valorização estimada — clica nas colunas para ordenar</p>", unsafe_allow_html=True)
        ...
        if top:
            ...
            for score in top:
                ...
                if val and val.valor_justo and l.preco_pedido:
    ...
    with st.expander("🏥 Estado do Pipeline", expanded=False):
        ...
        for label, key in [("💾 Base de Dados", "database"), ("🕷️ Scraping", "scraping"),
