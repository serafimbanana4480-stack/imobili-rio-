...
from realestate_engine.dashboard.utils.pagination import PaginationHandler, get_cached_data
...
def render_search():
    ...
    with col_preset1:
        if st.button(" Oportunidades", width="stretch"):
    ...
    with col_preset2:
        if st.button(" T2-T3", width="stretch"):
            st.session_state['tipologia'] = "T2"
            st.rerun()
    
    with col_preset3:
        if st.button(" Centro", width="stretch"):
    ...
    search_clicked = st.button(" Pesquisar", type="primary", width="stretch")
    if search_clicked or auto_search:
        ...
        # Apply pagination
        paginator = PaginationHandler(items_per_page=max_results)
        page_results = paginator.render_with_pagination(results, key_prefix="search")
        ...
        if page_results:
            ...
            df = pd.DataFrame(page_results)[display_cols]
            df.columns = [
            ...
            titles = [f"{r['Título']} - {r['Preço']:,.0f}€" for r in page_results[:20]]
