...
def render_debug_logs():
    ...
    #  Scraping Block Detection 
    st.markdown("---")
    st.markdown("###  Deteção de Bloqueios (Anti-Bot)")
    ...
    if zero_scrapes >= 3:
        st.error(f" {zero_scrapes} execuções de scraping consecutivas sem registos — possível bloqueio anti-bot")
    else:
