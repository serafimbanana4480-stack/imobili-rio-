...
from realestate_engine.dashboard.utils.formatting import format_currency, format_percentage
...
def render_investor_tools():
        ...
        with col2:
            location_tier = st.selectbox(
                ...
                format_func=lambda x: {
                    ...
                    "average": "➡️ Média (Campanha, Matosinhos)",
        ...
        if st.button("📊 Calcular ROI", type="primary", use_container_width=True):
            ...
            if roi:
                ...
                with col_x:
                    ...
                    st.write(f"Renda mensal estimada: **{format_currency(roi['estimated_monthly_rent'])}**")
                    st.write(f"Rendimento anual bruto: {format_currency(roi['annual_rent'])}")
                    st.write(f"Custos anuais: {format_currency(roi['annual_costs'])}")
                    st.write(f"Rendimento líquido anual: **{format_currency(roi['net_income'])}**")

                with col_y:
                    st.markdown("**📊 Investimento Total**")
                    st.write(f"Preço compra: {format_currency(purchase_price)}")
                    st.write(f"Custos aquisição (IMT+IS+Not.): {format_currency(roi['acquisition_costs'])}")
                    if renovation > 0:
                        st.write(f"Renovação: {format_currency(renovation)}")
                    st.write(f"**Investimento total: {format_currency(roi['total_investment'])}**")
                    st.write(f"Valor futuro ({holding_years}a): {format_currency(roi['future_value'])}")

                # Payback
                pb = roi["payback_years"]
                if pb < 100:
                    st.info(f"⏱️ Payback estimado: **{pb:.1f} anos**")
                else:
                    st.warning("⚠️ Rendimento líquido negativo — investimento não gera cash flow positivo")
                ...
                if total_roi > 50:
                    ...
                    st.info(f"➡️ ROI total em {holding_years} anos: **{total_roi:.0f}%** — Retorno moderado")
                else:
    ...
    with tab2:
        ...
        if st.button("🏦 Simular Crédito", type="primary", use_container_width=True):
            mortgage = InvestorTools.calculate_mortgage(m_price, m_down, m_rate, m_term)

            col_a, col_b, col_c = st.columns(3)
            with col_a:
                st.metric("💳 Prestação Mensal", format_currency(mortgage['monthly_payment']))
            with col_b:
                st.metric("💰 Valor Financiado", format_currency(mortgage['loan_amount']))
            with col_c:
                st.metric("📊 Entrada", format_currency(mortgage['down_payment']))

            st.markdown("---")
            st.write(f"Total pago ao banco: **{format_currency(mortgage['total_paid'])}**")
            st.write(f"Total de juros: **{format_currency(mortgage['total_interest'])}**")
            st.write(f"Rácio juros/capital: {mortgage['interest_to_principal_ratio']:.1f}x")

            if mortgage['interest_to_principal_ratio'] > 1.5:
                st.warning("⚠️ Vai pagar mais em juros do que o capital emprestado. Considere um prazo mais curto.")
    ...
    with tab3:
        ...
        if st.button("⚡ Analisar", type="primary", use_container_width=True):
            ...
            col_a, col_b, col_c, col_d = st.columns(4)
            with col_a:
                st.metric("€/m²", format_currency(price_m2))
            with col_b:
            ...
            if discount_vs_ine > 10:
                st.success(f"🔥 **{discount_vs_ine:.0f}% abaixo** da mediana INE ({format_currency(ine_median)}/m²) — Potencial oportunidade!")
            elif discount_vs_ine > 0:
                st.info(f"✅ {discount_vs_ine:.0f}% abaixo da mediana INE — Preço competitivo")
            elif discount_vs_ine > -10:
                st.warning(f"➡️ {abs(discount_vs_ine):.0f}% acima da mediana INE — Preço de mercado")
            else:
                st.error(f"⚠️ {abs(discount_vs_ine):.0f}% acima da mediana INE — Sobrevalorizado")
