...
def render_market_analysis():
    ...
    for l in listings:
        ...
        df_data.append({
            ...
            "area": l.area_util_m2 or l.area_bruta_m2 or 0,
    ...
    available_regions = sorted({
        ine._distrito_to_regiao.get(ine._concelho_to_distrito.get(str(conc).lower().strip(), ""), "norte")
        for conc in df["concelho"].dropna().astype(str).unique()
    })
    region_options = ["Todas"] + [region.title() for region in available_regions]
    selected_region = st.sidebar.selectbox("Região", region_options)

    if selected_region != "Todas":
        filtered_df = df[df["concelho"].str.lower().map(
            lambda conc: ine._distrito_to_regiao.get(ine._concelho_to_distrito.get(conc, ""), "norte").title()
        ) == selected_region]
    else:
        filtered_df = df.copy()

    available_concelhos = sorted(filtered_df["concelho"].dropna().astype(str).unique())
    concelho_options = ["Todos"] + available_concelhos
    selected_concelho = st.sidebar.selectbox("Concelho", concelho_options)
    if selected_concelho != "Todos":
        filtered_df = filtered_df[filtered_df["concelho"].str.lower() == selected_concelho.lower()]

    if filtered_df.empty:
        st.info("Sem dados para os filtros selecionados.")
        return

    scope_label = selected_concelho if selected_concelho != "Todos" else (selected_region if selected_region != "Todas" else "Portugal")
    st.title(f" Análise de Mercado — {scope_label}")
    ...
    with col1:
        st.metric(" Preço Mediano", f"{filtered_df['price'].median():,.0f}€".replace(",", "."))
    with col2:
        st.metric(" €/m² Mediano", f"{filtered_df['price_m2'].median():,.0f}€".replace(",", "."))
    with col3:
        st.metric(" Total Anúncios", f"{len(filtered_df):,}".replace(",", "."))
    with col4:
        st.metric(" Freguesias", filtered_df["freguesia"].nunique())
    with col5:
        top_portal = filtered_df["portal"].value_counts().index[0] if len(filtered_df) > 0 else "N/A"
    ...
    tab_resumo, tab_freguesia, tipologia_tab, ine_tab = st.tabs([
        " Resumo Geral", " Por Freguesia", " Por Tipologia", " INE Contexto"
    ])
    ...
    with tab_resumo:
        if not HAS_PLOTLY:
            ...
            with col_a:
                ...
                fig = px.histogram(
                    filtered_df[filtered_df["price_m2"] < 8000], x="price_m2", nbins=60,
                    labels={"price_m2": "€/m²"},
                ...
                fig.add_vline(x=filtered_df["price_m2"].median(), line_dash="dash",
                             annotation_text=f"Mediana: {filtered_df['price_m2'].median():,.0f}€")
    ...
    with tab_freguesia:
        if not HAS_PLOTLY:
            ...
            # Freguesia selector
            freguesias = sorted(filtered_df["freguesia"].unique())
            selected_freg = st.selectbox(" Seleciona a Freguesia", freguesias, index=0)

            freg_data = filtered_df[filtered_df["freguesia"] == selected_freg]
            ...
            freg_stats = filtered_df.groupby("freguesia").agg(
    ...
    #  Tab 4: INE Contexto 
    with ine_tab:
        st.markdown("####  Dados INE — Contexto Macroeconómico")
        ...
        for conc, data in sorted(ine.concelhos_data.items()):
            conc_listings = filtered_df[filtered_df["concelho"].str.lower() == conc.lower()]
