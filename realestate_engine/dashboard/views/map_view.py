...
def render_map_view():
    ...
    st.title("🗺️ Mapa de Propriedades")
    ...
    for l in valid_listings:
        ...
        map_data.append({
            ...
            "score_num": score.score_total if score else 0,
    ...
    if score_filter == "Imperdível (8+)":
        df = df[df['score_num'] >= 8.0]
    elif score_filter == "Excelente (7+)":
        df = df[df['score_num'] >= 7.0]
    elif score_filter == "Bom (5+)":
        df = df[df['score_num'] >= 5.0]
    ...
    tooltip = {
        ...
        "style": {"backgroundColor": "#1E293B" if is_dark_mode() else "white", "color": "#E2E8F0" if is_dark_mode() else "black"},
    }
