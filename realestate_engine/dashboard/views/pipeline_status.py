...
def render_pipeline_status():
    ...
    phases = [
        ("🕷️ Scraping", "spider_manager", "#3B82F6"),
        ("⚙️ ETL", "pipeline_etl", "#8B5CF6"),
    ...
    st.info(
        "ℹ️ O pipeline corre automaticamente em loop pelo Orchestrator (24/7). "
        "Para execução manual com feedback ao vivo (logs, progresso por portal, "
        "novos vs duplicados), usa a página **🔄 Scraping**."
    )
    col1, col2 = st.columns(2)
    with col1:
        if st.button("▶️ Ir para Scraping (execução manual)", type="primary", use_container_width=True):
            st.session_state['page'] = "⚙️ Sistema"
