...
async def test_local_fallback_thesis_when_provider_not_implemented():
    ...
    assert "Fallback local ativado" in thesis or "fallback" in thesis.lower()
    # Strategy section may vary (random.choice) — check for any reliable keyword
    assert any(kw in thesis.lower() for kw in ("buy-to-let", "comparáveis", "visita", "renda", "mercado"))
