...
class TestScoringEngine:
    ...
    def test_score_includes_bert_sentiment_features(self):
        engine = ScoringEngine()
        listing = CleanListing(
            source_portal="idealista",
            source_id="bert-1",
            source_url="https://idealista.pt/bert-1",
            scrape_timestamp="2024-01-01",
            titulo="T2 Lisboa",
            ...
            estado="usado",
            tipologia="T2",
            bert_sentiment_score=0.9,
            bert_sentiment_label="POSITIVE",
        )

        result = engine.score(listing, valuation=None)
        assert 0 <= result["score_condition"] <= 10
        # 'usado' is 4.5. Positive sentiment adds 0.3 + 0.1 = 4.9.
        assert result["score_condition"] >= 4.9
